...
int ObtenerAltura(int nodo_idx) {
    if (nodo_idx == -1 || nodo_idx >= (int)alturas.size()) return 0;
    return alturas[nodo_idx];
}

int ObtenerBalance(int nodo_idx) {
    if (nodo_idx == -1) return 0;
    return ObtenerAltura(izq[nodo_idx]) - ObtenerAltura(der[nodo_idx]);
}

void ActualizarAltura(int nodo_idx) {
    if (nodo_idx != -1) {
        alturas[nodo_idx] = 1 + max(ObtenerAltura(izq[nodo_idx]), ObtenerAltura(der[nodo_idx]));
    }
}

int RotarDerecha(int y) {
    int x = izq[y];
    int T2 = der[x];
    der[x] = y;
    izq[y] = T2;
...
// --- INSERCIÓN AVL ---
int InsertarAVL(int nodo_idx, int valor, int nuevo_idx) {
    if (nodo_idx == -1) return nuevo_idx;

    if (valor < valores[nodo_idx]) {
        izq[nodo_idx] = InsertarAVL(izq[nodo_idx], valor, nuevo_idx);
    } else { 
        der[nodo_idx] = InsertarAVL(der[nodo_idx], valor, nuevo_idx);
    }

    ActualizarAltura(nodo_idx);
    int balance = ObtenerBalance(nodo_idx);

    if (balance > 1 && valor < valores[izq[nodo_idx]]) return RotarDerecha(nodo_idx);
    if (balance < -1 && valor >= valores[der[nodo_idx]]) return RotarIzquierda(nodo_idx);
    if (balance > 1 && valor >= valores[izq[nodo_idx]]) {
        izq[nodo_idx] = RotarIzquierda(izq[nodo_idx]);
        return RotarDerecha(nodo_idx);
    }
    if (balance < -1 && valor < valores[der[nodo_idx]]) {
        der[nodo_idx] = RotarDerecha(der[nodo_idx]);
        return RotarIzquierda(nodo_idx);
    }
...
int BorrarAVL(int nodo_idx, int valor) {
    if (nodo_idx == -1 || nodo_idx >= (int)valores.size()) return -1;

    if (valor < valores[nodo_idx]) {
        izq[nodo_idx] = BorrarAVL(izq[nodo_idx], valor);
    } else if (valor > valores[nodo_idx]) {
        der[nodo_idx] = BorrarAVL(der[nodo_idx], valor);
    } else {
        if (izq[nodo_idx] == -1) {
            return der[nodo_idx];
        ...
        der[nodo_idx] = BorrarAVL(der[nodo_idx], valores[min_idx]);
    }

    if (nodo_idx == -1) return nodo_idx;
    ActualizarAltura(nodo_idx);
    int balance = ObtenerBalance(nodo_idx);

    if (balance > 1 && ObtenerBalance(izq[nodo_idx]) >= 0) return RotarDerecha(nodo_idx);
    if (balance > 1 && ObtenerBalance(izq[nodo_idx]) < 0) {
        izq[nodo_idx] = RotarIzquierda(izq[nodo_idx]);
        return RotarDerecha(nodo_idx);
    }
    if (balance < -1 && ObtenerBalance(der[nodo_idx]) <= 0) return RotarIzquierda(nodo_idx);
    if (balance < -1 && ObtenerBalance(der[nodo_idx]) > 0) {
        der[nodo_idx] = RotarDerecha(der[nodo_idx]);
        return RotarIzquierda(nodo_idx);
    }
...
DWORD WINAPI HiloInsercion(LPVOID lpParam) {
    ...
    valores.push_back(val); izq.push_back(-1); der.push_back(-1); alturas.push_back(1);
    raiz = InsertarAVL(raiz, val, (int)valores.size() - 1);
...
void DibujarArbol(HDC hdc, int nodo_idx, float x, float y, float desp_x) {
    ...
    if (izq[nodo_idx] != -1) {
        ...
        DeleteObject(hPen); DibujarArbol(hdc, izq[nodo_idx], x - desp_x, y + 80, desp_x * 0.5f);
    }
    if (der[nodo_idx] != -1) {
        HPEN hPen = CreatePen(PS_SOLID, 2, RGB(160, 160, 160)); SelectObject(hdc, hPen);
        MoveToEx(hdc, (int)x, (int)y, NULL); LineTo(hdc, (int)(x + desp_x), (int)(y + 80));
        DeleteObject(hPen); DibujarArbol(hdc, der[nodo_idx], x + desp_x, y + 80, desp_x * 0.5f);
    }

    HBRUSH hBrush = CreateSolidBrush(RGB(155, 89, 182)); HPEN hPenNodo = CreatePen(PS_SOLID, 1, RGB(155, 89, 182));
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
        ...
        case WM_PAINT: {
            ...
            EnterCriticalSection(&cs);
            if (raiz != -1) { RECT rect; GetClientRect(hwnd, &rect); DibujarArbol(hdc, raiz, rect.right / 2.0f, 130.0f, rect.right / 4.0f); }
            LeaveCriticalSection(&cs);
            EndPaint(hwnd, &ps);
            break;
        }
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR args, int ncmdshow) {
    ...
    wc.hInstance = hInst; wc.lpszClassName = L"MiClaseAVLDef"; wc.lpfnWndProc = WindowProcedure;
    if (!RegisterClassW(&wc)) return -1;
    hVentanaPrincipal = CreateWindowW(L"MiClaseAVLDef", L"Árbol AVL Dinámico - Fondo Blanco con Borrado", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 100, 100, 1024, 720, NULL, NULL, hInst, NULL);
    MSG msg = {0};
